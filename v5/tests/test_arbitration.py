...
def test_l2_suppresses_judge_on_same_claim():
    l2 = _verdict("L2", "c1", severity="high")
    judge = _verdict("JUDGE", "c1")
    arbitrate([l2, judge])
    assert l2.fired and not l2.suppressed_by
    assert not judge.fired
    assert judge.suppressed_by == ["L2"]
    assert judge.evidence.extra["fired_before_suppression"] is True
...
def test_distinct_claims_unaffected():
    l2 = _verdict("L2", "c1")
    judge = _verdict("JUDGE", "c2")
    arbitrate([l2, judge])
    assert l2.fired and judge.fired
    assert not l2.suppressed_by and not judge.suppressed_by


def test_never_two_firing_verdicts_per_claim():
    verdicts = [_verdict("L2", "c1"), _verdict("JUDGE", "c1"), _verdict("JUDGE", "c2")]
    arbitrate(verdicts)
